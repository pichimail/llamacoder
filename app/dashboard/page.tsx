...
export default async function DashboardPage() {
  ...
  if (!user) {
    return (
      <main className="min-h-dvh bg-muted/20 px-4 py-8 text-foreground md:px-8">
        <div className="mx-auto max-w-5xl">
          <Card className="overflow-hidden rounded-3xl border-border/70">
            <CardContent className="grid gap-6 p-6 md:grid-cols-[1fr_auto] md:p-8">
              <div>
                <Badge variant="secondary" className="rounded-full">Workspace dashboard</Badge>
  ...
  return (
    <main className="min-h-dvh bg-muted/20 px-4 py-6 text-foreground md:px-8">
        ...
        <section className="overflow-hidden rounded-3xl border border-border/70 bg-card">
          <div className="grid gap-6 p-5 md:grid-cols-[1fr_auto] md:p-7">
            ...
            <div className="flex flex-wrap items-end gap-2">
              <Button asChild variant="outline" className="rounded-xl"><Link href="/settings"><Settings2 className="size-4" />Settings</Link></Button>
              <Button asChild className="rounded-xl"><Link href="/"><Sparkles className="size-4" />New build</Link></Button>
            </div>
          </div>
        </section>
              ...
              {projects.length === 0 ? (
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  {projects.map((project) => (
                    <Link key={project.id} href={`/projects/${project.id}`} className="group rounded-2xl border border-border bg-background p-4 transition hover:border-foreground/30 hover:shadow-sm">
                      ...
                      <div className="mt-4 flex flex-wrap gap-2 text-xs text-muted-foreground">
                        <Badge variant="outline" className="rounded-full">{project._count.chats} chats</Badge>
                        <Badge variant="outline" className="rounded-full">{project._count.members} members</Badge>
        ...
        <Card className="rounded-3xl">
          ...
          <CardContent className="grid gap-2">
            {recentChats.length === 0 ? (
              ...
              <Link key={chat.id} href={`/chats/${chat.id}`} className="grid gap-2 rounded-2xl border border-border bg-background p-4 transition hover:border-foreground/30 md:grid-cols-[1fr_auto] md:items-center">
                <div className="min-w-0">
                ...
                <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground md:justify-end">
                  <Badge variant="secondary" className="rounded-full">{chat.project?.name || "Project"}</Badge>
...
function MetricCard({ icon: Icon, label, value, href }: { icon: React.ComponentType<{ className?: string }>; label: string; value: number; href: string }) {
  return (
    <Link href={href} className="group block rounded-3xl border border-border/70 bg-card p-4 transition hover:border-foreground/30 hover:shadow-sm">
      <div className="flex items-center justify-between gap-3">
        <div className="rounded-2xl border border-border bg-background p-2 text-muted-foreground transition group-hover:text-foreground"><Icon className="size-4" /></div>
